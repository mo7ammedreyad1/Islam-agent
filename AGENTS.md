...
## 6. المرونة الهندسية والذكاء البرمجي (Dynamic & Adaptive Execution)

الـ Agent المسؤول عن قراءة هذا المستند وبناء كود المشهد (`scene.html`) مكلّف بالتفكير الهندسي المنطقي والتكيف الذكي مع كافة معطيات التصميم، مع تطبيق القواعد العامة التالية على أي عنصر بصري:

* **التكيف الديناميكي للأبعاد والحجم (Dynamic Sizing & Layout):**
  * يُمنع استخدام القيم الثابتة الصارمة (Hardcoded Values) إذا كانت ستتسبب في تداخل العناصر، اقتطاع النصوص (Overflow)، أو خروج الإخراج عن المتطلبات البصرية.
  * كل حاوية، مربع نص، شريط خيارات، أو عنصر زخرفي يجب أن تُحسب أبعاده، مسافاته الداخلية (Paddings)، وموقعه الهيكلي ديناميكيًا بناءً على حجم وطول المحتوى الفعلي المدمج فيه.

* **الحفاظ على التوازن والتماثل البصري (Visual Balance & Spacing):**
  * عند وجود عناصر متصلة ببعضها حركياً أو بصرية (مثل: نصوص محصورة بين خطوط زخرفية، أزرار بجانبها أيقونات، أو حقول نصية متتابعة)، يجب أن تتعدل المسافات الفاصلة (Gaps & Offsets) تلقائياً للحفاظ على النسب الذهبية والتمركز الصحيح، بغض النظر عن تغير طول المحتوى.

* **مرونة القرار الفني للـ Agent:**
  * يملك الـ Agent الصلاحية الكاملة لإعادة توزيع المساحات، ضبط أحجام الخطوط، أو إزاحة محاور الـ (X/Y) إذا تطلب المحتوى الفعلي ذلك، بهدف حماية المشهد من التداخل الضار وتوفير تجربة بصرية فائقة الدقة والاحترافية.
  * المنطق البصري السليم وراحة العين أثناء القراءة والمشاهدة هي المعيار الأول الحاكم لكتابة الكود.
