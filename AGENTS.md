...
2. **اقرأ ملف الهوية المطلوب بالكامل** — مش قراءة سريعة، اقرأه أكتر من مرة لو
   ...
   المحتوى المطلوب توفيره** (الحقول اللي وثّقتها الهوية بالذات في فقرة الوصف —
   الأسماء والبنية حرة تمامًا لكل هوية، مفيش شكل موحّد مفروض من العقد التقني
   العام)، والدالتين الإلزاميتين (`prepareIdentity`, `drawSceneAtTime`).
...
1. **🎨 طبقة الهوية (Identity Layer)** — **self-contained بالكامل**: بتُكتب من
   الصفر بناءً على فهمك لملف `identities/<اسم-الهوية>.md` اللي المهمة حددته
   (تنفيذ مرجعي، مش كتلة تُنسخ — راجع "فهم عميق قبل الكتابة" تحت)، مع **قيم
   بيانات المحتوى الحقيقية** لتطابق المهمة الحالية (السورة، القارئ، نص الآيات،
   أو أي شكل محتوى تاني حسب الهوية — راجع "عقد ملف هوية `.md`" تحت). فيها
   بيانات المحتوى، الألوان، الخطوط، الـ CSS، منطق جلب أي أصول الهوية محتاجاها
   (صوت/صورة/فيديو B-roll)، ومنطق الرسم/الحركة على الـ canvas.
...
ملف `identities/<اسم>.md` هو **تنفيذ مرجعي كامل (Reference Implementation)**،
مش وصف كلامي عام، **self-contained بالكامل** — فيها بيانات المحتوى، جلب
الأصول، والتصميم مع بعض. **الكود جواه بيوضح الفكرة والسلوك المطلوب، مش كتلة
تُنسخ حرفيًا** — لما تكتب `scene.html` الفعلي، اقرأه كويس وافهمه، وبعدين
**اكتب تنفيذك الخاص بيك من الصفر** بناءً على الفهم ده (نفس الألوان والتخطيط
والحركة والسلوك، لكن الكود نفسه بتاعك إنت) — وده اللي بيدّيك المرونة إنك
تتصرف لو المستخدم طلب صراحة يضيف/يشيل/يغيّر حاجة في الهوية. **ميحتويش على أي
حاجة من الطبقة التقنية تحت (مفيش Mediabunny، مفيش render hooks، مفيش محرك
تصدير)** — دول موجودين مرة واحدة بس في الطبقة التقنية الثابتة، وتكرارهم جوه
ملف الهوية خطأ في حد ذاته (تكرار كود بيصعّب الصيانة، ولو حصل تعارض بينه وبين
النسخة التانية هيبقى مصدر باگ غامض).
...
5. **كود JS الهوية بالكامل — self-contained من غير أي اعتماد على متغيرات من
   طبقة تانية**. الكود هنا (دوال الرسم، الـ `PALETTE`، شكل `CONFIG`، أسماء
   الخطوط، ومنطق جلب الأصول جوه `prepareIdentity()`) **كله تنفيذ مرجعي يوضح
   الفكرة والسلوك المطلوب — مش كتلة تُنسخ حرفيًا بمتغيراتها**. لما تكتب
   `scene.html` الفعلي:
   - **دوال الرسم/`PALETTE`/شكل `CONFIG`/أسماء الخطوط**: افهمها كويس واكتب
     نسختك الخاصة بيك من نفس الألوان والتخطيط والحركة — ده اللي بيدّيك
     المرونة تتصرف لو المستخدم طلب صراحة يضيف عنصر تصميم جديد، يشيل حاجة
     موجودة، أو يغيّر تفصيلة معيّنة.
   - **منطق جلب الأصول جوه `prepareIdentity()`**: اكتبه من جديد بقيم حرفية
     محلولة بالكامل لكل مقطع (راجع "قيم المحتوى تُكتب حرفية دايمًا" فوق)، مش
     بنفس الـ template variables الموضّحة هنا.
   - **قيم قسم "بيانات المحتوى" بس** (زي `SURAH_NUMBER`, `SURAH_VERSES`) هي
     اللي بتتغيّر لكل مهمة (نتيجة `curl`/طلب المستخدم الفعلي — راجع القسم 4،
     قاعدة "ممنوع الكتابة من الذاكرة")، بنفس ترتيب الأقسام دي:
...
7. **العقد بين الهوية والطبقة التقنية — أقل ما يمكن، عمدًا**: الهوية لازم
   ...
   **باقي أي حاجة داخلية للهوية — شكل بيانات المحتوى، طريقة جلب الأصول، أسماء
   ...
   التقنية خالص.**
8. **ملاحظات/تنبيهات خاصة بالهوية دي بس** (لو فيه) — **قبل ما تستخدم أي هوية،
   افتح ملفها وشوف القسم ده جواه.** لو ملف هوية جديد ملوش القسم ده لسه، معنى
   كده مفيش ملاحظات معروفة عليه — كمّل عادي بالقواعد العامة في الأقسام اللي
   فوق بس. لو مشكلة جديدة اتحلت وهي مرتبطة بتصميم/محتوى هوية معيّنة بالذات
   (مش بالطبقة التقنية الثابتة، اللي مكانها القسم 3)، ضيفها هنا جوه ملف
...
<body>
    ...
    <script type="module">
        ...
        // فهمك لـ identities/<اسم>.md، ببيانات محتوى حقيقية للمهمة الحالية) ============
