# هوية الـ Agent + العقد التقني الإلزامي لفيديوهات الشورتس متعددة المجالات
...
الـ Agent مسؤول عن إنتاج فيديوهات شورتس **في أي مجال محتوى** (تعليمي، ترفيهي،
إخباري، ديني، تسويقي، أو أي مجال تاني — المجال والشكل بالكامل قرار ملف الهوية
المستخدمة من `identities/`، مفيش مجال "افتراضي" مفروض) بشكل كامل من الصفر: جلب
النص والصوت (لو المحتوى محتاج صوت) من مصادر موثوقة، كتابة `scene.html` يلتزم
بالعقد التقني في القسم 2، رندره، ثم رفعه على GitHub Release وتوثيقه.
...
بيانات المحتوى اللي الهوية دي بتحتاجه** (مش مفروض عليها شكل موحّد ولا مجال
معيّن — هوية ممكن تكون آية قرآنية بتفسير، وهوية تانية معلومة علمية قصيرة، وهوية
تالتة نصيحة طبخ، وهوية رابعة خبر عاجل، وهوية خامسة صوت بس من غير أي نص خالص —
**أي مجال محتوى صالح، الديني مجرد مثال واحد من أمثلة كتير ممكنة**)، **ومنطق
جلب أي أصول خاصة بيها** (صوت، صورة، فيديو B-roll، أو ولا حاجة خالص لو مش
محتاجة). **مرجع شامل ومستقل تمامًا (Self-contained) — Blueprint هندسي كامل
يُفهم ويُبنى عليه** (فيها كود JS/CSS فعلي، مش وصف كلامي عام)، لكن **بدون أي
جزء من render hooks أو محرك التصدير (Mediabunny) نفسه**، لأن ده كله ثابت
وموجود مرة واحدة بس في "الطبقة التقنية الثابتة" بالقسم 2 ومالهاش أي علاقة
بمجال محتوى معيّن. **المجلد ده فيه — وهيفضل يتزود فيه مع الوقت — أكتر من هوية
مختلفة تمامًا (مش بس بصريًا، كمان في مجالها وشكل المحتوى اللي بتحتاجه).** مفيش
هوية "افتراضية" أو "أساسية" بينهم — كل ملف فيه مرجع حرفي مستقل بذاته وكافي
وحده.
...
3. **اكتب `scene.html`** بدمج الطبقتين بالترتيب في القسم 2 — بفهم هندسي كامل لما
   بتكتبه (راجع "فهم عميق قبل الكتابة" تحت)، **لا تجميع نصوص أعمى ولا نسخ حرفي**:
   طبقة الهوية بالكامل (مبنية بفهم كامل على أساس كود ملف الـ `.md` — باعتباره
   Blueprint ومنطق أساسي يُستوعب أولًا، لا نصًا يُلصق حرفيًا — إلا قيم بيانات
   المحتوى بس، اللي بتستبدلها بالقيم الحقيقية للمهمة الحالية نتيجة `curl` فعلية،
   راجع القسم 4)، ثم الطبقة التقنية الثابتة (مطابقة وظيفيًا للمرجع الثابت في
   القسم 2، مكتوبة بفهم كامل لتفاعل حلقة التصدير ومعالجة الصوت/الفيديو، من غير
   أي تعديل يُخل بالعقد التقني).
...
- **تفاصيل الهوية البصرية نفسها** (الألوان، الخطوط، تفاصيل التخطيط، الحركات،
  ...
  شوية"). الهدف إنك تكون مرن مع طلب المستخدم فوق أساس الهوية، مش إنك تعيد
  إنتاجها بفهمك الخاص من غير وعي بكل تفصيلة صغيرة نصّت عليها الهوية، ومش إنك
  تغيّر فيها من عندك من غير ما المستخدم يطلب.
...
- **فحص إلزامي لصحة `scene.html` قبل تشغيل سكريبت الرندر الكامل** (مش بعده):
  ...
  (async () => {
    ...
    await page.goto(`http://localhost:${port}/scene.html`, { waitUntil: 'domcontentloaded', timeout: 60000 });
...
1. **🎨 طبقة الهوية (Identity Layer)** — **self-contained بالكامل**: مبنية
   **بفهم كامل** (كود، مش وصف) على أساس ملف `identities/<اسم-الهوية>.md` اللي
   المهمة حددته — باعتباره Blueprint هندسي ومنطق أساسي يُستوعب قبل الكتابة، لا
   نصًا يُلصق حرفيًا — وبعدين **قيم بيانات المحتوى بس** بتتعدّل لتطابق المهمة
   الحالية (السورة، القارئ، نص الآيات، أو أي شكل محتوى تاني حسب الهوية — راجع
   "عقد ملف هوية `.md`" تحت). فيها بيانات المحتوى، الألوان، الخطوط، الـ CSS،
   منطق جلب أي أصول الهوية محتاجاها (صوت/صورة/فيديو B-roll)، ومنطق الرسم/الحركة
   على الـ canvas.
2. **⚙️ الطبقة التقنية الثابتة (Fixed Technical Layer)** — **موجودة كاملة تحت
   في القسم ده، ومالهاش أي علاقة بمجال محتوى معيّن.** فيها
   Mediabunny، الـ render hooks، أدوات جلب أصول عامة (صوت/فيديو B-roll)، وحلقة
   التصدير. **تُكتب مطابقة وظيفيًا للمرجع الثابت ده، بفهم هندسي كامل لتفاعل
   حلقة التصدير ومحرك الرسم على الـ Canvas ومعالجة الصوت/الفيديو، من غير أي
   تعديل يُخل بالعقد التقني** — دي بالظبط النقطة اللي بتضمن إن أي هوية جديدة
   (حتى لو منطقها ومحتواها مختلف تمامًا عن أي هوية موجودة) هتشتغل مع الرانر من
   غير مفاجآت.
...
### فهم عميق قبل الكتابة — مش تجميع نصوص أعمى
...
**لا وجود للنسخ واللصق الأعمى**: القواعد في `AGENTS.md` وكود كل ملف
`identities/*.md` مش نصوص تتلزّق تلقائيًا مع بعض من غير وعي — دي **مخطط هندسي
(Blueprint) ومنطق أساسي (Core Logic)** لازم تفهمه وتستوعبه كويس أولًا، وبعدين
**تكتب `scene.html` كامل بإيدك وبوعي هندسي متكامل**: تفهم إزاي حلقة
التصدير (Render Loop) بتشتغل، وإزاي محرك الرسم على الـ Canvas (Canvas Hooks)
بيتفاعل معاها، وإزاي معالجة الصوت/الفيديو بتتم فعليًا — وتكتب الكود بأسلوب
نظيف وعالي الجودة يحقق العقد التقني وظيفيًا بالكامل. الفهم ده هو اللي بيخليك —
لما يحصل خطأ — تعرف تشخّصه من رسالته مباشرة (زي القسم 3 بيوضح، اللي فيه أخطاء
حصلت فعليًا في تشغيلات سابقة ولازم تاخد بالك منها وانت بتكتب، مش بس تنسخها
كنص) بدل ما تستنى `TimeoutError` غامض وتخمّن.

---

### 🎨 عقد ملف هوية `.md` — إيه اللي لازم يوفّره
ملف `identities/<اسم>.md` هو **مرجع شامل ومستقل تمامًا (Self-contained) —
Blueprint هندسي كامل يُفهم ويُبنى عليه**، مش وصف كلامي عام — فيها بيانات
المحتوى، جلب الأصول، والتصميم مع بعض. **ميحتويش على أي حاجة من الطبقة التقنية تحت (مفيش Mediabunny، مفيش
...
4. **أبعاد الكانفاس** (`width`/`height` اللي هتتحط في `<canvas>` في الـ HTML)،
   **ولغة/اتجاه نص الفيديو** (`lang`/`dir` اللي هتتحط في وسم `<html>`، وعنوان
   الصفحة المناسب — أي هوية بأي لغة، مش بس العربي).
5. **كود JS الهوية بالكامل — self-contained من غير أي اعتماد على متغيرات من
   طبقة تانية**. الكتلة دي كود مرجعي (Blueprint) يُفهم أولًا ثم يُبنى عليه في
   `scene.html` (البنية والأسماء)، وبعدين **قيم قسم "بيانات المحتوى" بس** هي
   اللي بتتعدّل لكل مهمة (نتيجة
...
### ⚙️ الطبقة التقنية الثابتة — الهيكل الكامل، مرجع هندسي يُفهم بالكامل ثم يُكتب مطابقًا وظيفيًا
ده الجزء اللي **ثابت وظيفيًا 100%** في كل `scene.html` — بيتكتب بفهم هندسي كامل
لتفاعل كل جزء فيه (حلقة التصدير، الـ render hooks، معالجة الصوت/الفيديو)، وأي
تعديل يُخل بوظيفته (حتى لو شكله بسيط أو "تحسين") يعتبر مخالفة للعقد التقني.

**هيكل الـ `<head>`/`<body>` الثابت** (الأماكن المعلّمة بـ 🎨 بتتملى من ملف الهوية
— بما فيها `lang`/`dir` في وسم `<html>` وعنوان الصفحة، اللي بيحددهم كل هوية حسب
لغتها واتجاه نصها، مش قيمة ثابتة مفروضة على كل الهويات):
...
<body>
    ...
    <script type="module">
        ...
        // ============ 🎨 طبقة الهوية بالكامل — هنا (مبنية بفهم كامل على
        // أساس identities/<اسم>.md، بيانات المحتوى داخلها معدَّلة لتطابق
        // المهمة الحالية فقط) ============

        // ============ ⚙️ الطبقة التقنية الثابتة — من هنا تحت، مطابقة وظيفيًا للمرجع الثابت ============
    </script>
</body>
</html>
```
...
### دليل كتابة سكريبت الرندر — مرجع مُختبَر وشغّال فعليًا، يُفهم ويُكتب مطابقًا له وظيفيًا
...
> ⚠️ **قيم الـ timeout الصريحة تحت (على `page.goto` وحلقة مراقبة التقدّم) إلزامية،
> مش اختيارية ومش قابلة للحذف أو التقليل.** الـ default بتاع Playwright لكل
> عملية شبكة (بما فيها `page.goto`) هو 30 ثانية بس — وتحميل الصفحة بيحتاج شبكة
> برّه (خطوط، أيقونات، استيراد Mediabunny)، ورندر فيديو فعلي بياخد دقايق. لو
> سبت أي واحدة من العمليتين دول من غير `timeout` صريح، السكريبت هيكراش بـ
> `TimeoutError: Timeout 30000ms exceeded` قبل ما الرندر يخلص أصلًا أو حتى قبل
> ما يبدأ، حتى لو باقي الكود كله صح 100%. اكتب السكريبت اللي تحت **بفهم كامل
> لكل جزء فيه**، مطابقًا له وظيفيًا بالكامل من غير حذف أو اختصار، وخصوصًا الـ
> `timeout` على `page.goto` والـ `deadline` في حلقة المراقبة.
>
...
> **ممنوع `grep` أو أي بحث جزئي هنا**. لو فيه أكتر من عملية شبكة (`page.goto`،
> `waitForFunction`، إلخ) في نفس الملف، لازم **كل واحدة منهم** تحمل `timeout`
> صريح أطول من الـ 30 ثانية الافتراضية — التأكد من وجود قيمة `timeout` مرة
> واحدة في الملف مش كافي، ممكن يكون فيه نداء شبكة تاني من غير أي `timeout`
> وبيقع على الـ 30 ثانية الافتراضية ويكراش قبل ما يوصل للنداء الصح خالص — وده
> بالظبط سبب إن القراءة لازم تكون كاملة، مش بحث عن كلمة واحدة.
...
(async () => {
  ...
  // waitUntil: 'domcontentloaded' بدل الـ default ('load') — الصفحة بتدير جاهزيتها بنفسها
  // عن طريق window.renderStatus، فمفيش داعي إن page.goto نفسه يستنى تحميل كل الموارد
  // الخارجية (خطوط/أيقونات/استيراد Mediabunny) قبل ما يكمل.
  await page.goto(`http://localhost:${port}/scene.html?autorender=true`, {
    waitUntil: 'domcontentloaded',
    timeout: 60000
  });

  // مراقبة تقدّم حية بدل انتظار أعمى واحد طويل: بتطبع status/progress كل ما يتغيّروا
  // (أو كل 3 ثواني كحد أقصى)، فيبان تقدّم الرندر لحظة بلحظة بدل استنى في العتمة
  // لحد ما الـ 8 دقايق تخلص أو الرندر يخلص.
  ...
  while (Date.now() < deadline) {
    ...
    const logLine = `status=${status} progress=${progress ?? '—'}`;
    if (logLine !== lastLogged) {
  ...
  if (!finalStatus) {
    const stuckStatus = await page.evaluate(() => window.renderStatus);
    throw new Error(`TimeoutError: renderStatus لسه "${stuckStatus}" بعد 8 دقايق من غير completed/error`);
  }
...
- بعد ما ترفع الفيديو وملف وصفه **و`scene.html` نفسه** فعليًا على الـ Release
  (بأمر `gh release upload` حقيقي ناجح لكل الثلاثة، مش افتراض)، اكتب:
  `video_<معرّف فريد للفيديو>_done.json` (المعرّف ممكن يكون رقم ترتيب الفيديو
  في المهمة، أو جزء مميز من `OUTPUT_FILENAME` — أي حاجة فريدة تكفي، مفيش شكل
  موحّد مفروض) يحتوي
...
1. **جلب الصوت**: `fetchAndDecodeAudio()` (الطبقة التقنية) بترفض تلقائيًا أي
   ...
   دلوقتي (مش مسؤولية كل هوية تكررهم أو تتذكرهم)، عشان مصدر صوت بيرجّع صفحة
   خطأ صغيرة بدل الملف الحقيقي (رابط `http://` أو ريدايركت غير متوقَّع) يبان
   فورًا كرسالة واضحة (`throw`) بدل فشل صامت بعدها بخطوات.

2. **متغيرات البيئة `$RELEASE_TAG` و`$GH_REPO`**: متاحين فعليًا الآن في أي أمر
...
3. **تحقق دايمًا من أي ملف نزّلته قبل ما تفترض إنه صح** — سواء صوت أو JSON أو صورة —
   بأمر بسيط زي `ls -la` أو `head -c 200 <file>`. عادة أرخص بكتير من اكتشاف المشكلة
   بعد خطوات كتير.

4. **ممنوع منعًا باتًا تنفيذ `node agent.js` كأمر terminal من جوه جلستك، بأي args**.
...
5. **`scene.html` لازم يتفتح دايمًا عن طريق سيرفر HTTP محلي، مش `file://`** — حتى لو
...
6. **قناة المتصفح لازم تكون `chrome` صراحة** في `chromium.launch({ channel: 'chrome' })`
   — لأن الـ workflow بيثبّت القناة دي بس (`npx playwright install --with-deps chrome`)،
   مش الـ Chromium الافتراضي.

7. **قبل ما تعتبر المهمة خلصت، تأكد إن كل قيم بيانات المحتوى المكتوبة فعليًا في
   `scene.html` (حسب شكل محتوى الهوية المستخدمة) بتطابق فعليًا المطلوب في
   المهمة، ومتطابقة مع بعضها البعض** (متبقاش مثلًا نص مقطع وصوت مقطع تاني خالص).
   الفيديو ممكن يتصدّر بنجاح كامل من غير أي خطأ في اللوج، ومع ذلك يحمل محتوى غلط
   تمامًا — الرندر مش هيكشفلك الغلطة دي لوحده أبدًا.

8. **ممنوع تحميل أي صوت أو صورة أو فيديو B-roll بـ `curl` حتى للتجربة/التأكد إن
...
1. **ممنوع منعًا باتًا** كتابة أي نص أو بيانات المفروض تكون حقيقية أو منسوبة
   لمصدر حقيقي (اقتباس، آية، حديث، معلومة، إحصائية، خبر، أو أي نص/بيانات تانية
   من النوع ده) من "معرفتك" الداخلية. كل نص/بيانات من النوع ده في أي
   `scene.html` لازم مصدرها نتيجة `curl`/طلب فعلي نُفّذ في نفس الجلسة على مصدر
   موثوق. **القرار إن محتوى هوية معيّنة محتاج تحقق من مصدر خارجي من عدمه (زي
   محتوى إبداعي بحت مالوش "مصدر حقيقي" يترجع له أصلًا) شأن ملف الهوية نفسه
   يوثّقه صراحة** — لكن أي محتوى موثّق كحقيقي/منسوب لمصدر يفضل تحت القاعدة دي
   بلا استثناء.
2. **لو الهوية المستخدمة فيها صوت**: كل توقيت مرتبط بالصوت (متى يظهر كل مقطع
   نص، متى تظهر أي بطاقة إضافية) يُحسب من **المدة الفعلية** للصوت بعد تحميله
   وفكّه (عن طريق `fetchAndDecodeAudio`/`concatenateAudioBuffers`)، وليس
   تخمينًا.
3. **مصدر الصوت (لو الهوية فيها صوت) قرار الهوية نفسها بالكامل** — موثّق في
   وصف ملف `identities/<اسم>.md` بتاعها (كل هوية بتحدد مصدرها الخاص حسب
   محتواها، مفيش مصدر واحد مفروض على كل الهويات). لازم يُجلب وقت التشغيل عن
   طريق `fetchAndDecodeAudio` (رابط `https://` مباشر، مش تحميل محلي مسبق).
4. **ملف وصف الفيديو الناتج** (اللي بيترفع مع كل فيديو على الـ Release — مختلف
   ...
   لمحتوى الفيديو الفعلي (حسب نوعه ومجاله — أي تفاصيل مناسبة لشكل المحتوى،
   زي اسم المصدر وعدد المقاطع والراوي لو محتوى مقروء بصوت، أو أي تفاصيل مكافئة
   لمجال تاني)، المدة الكلية، هل فيه شرح/تفسير إضافي أم لا، ورابط الـ GitHub
   Release الفعلي بعد الرفع.
5. **`scene.html` نفسه لازم يُرفع كـ asset على الـ Release مع الفيديو وملف
   الوصف — مش يفضل محليًا بس** (بأمر `gh release upload` حقيقي، بنفس أسلوب
   رفع الفيديو والوصف). الهدف إتاحة الكود اللي أنتج الفيديو بالظبط للمراجعة أو
   إعادة الاستخدام لاحقًا، مش بس الناتج النهائي.
