إليك البرومبت الشامل والمحدَّث لـ **AGENTS.md** بعد حذف كل ما يخص مجلد video-identities/*.md بالكامل، وتوضيح طبيعة ملفات الهوية داخل identities/*.md بأنها ملفات شاملة توصف الأصول المطلوبة (صوت، فيديو، صور...) وشكل البيانات وكود الرسم والتنفيذ بالكامل:
...
تم إلغاء وفصل أي مجلدات سابقة (مثل video-identities/)، وأصبحت **جميع الهويات معرّفة بالكامل وبشكل موحّد وشامل داخل مجلد identities/*.md فقط**.
**ملف الـ .md داخل مجلد identities/ ليس مجرد وصف بصري أو دليلاً لشكل الرسم فقط!** بل هو مرجع شامل ومستقل تمامًا (Self-contained) يحدد كل ما يخص الهوية حرفيًا:
 ...
 5. **الكود البرمجي التنفيذي (JS/CSS)**: يحتوي على كود شغال وجاهز للنسخ المباشر يتضمن الدالتين الإلزاميتين (prepareIdentity و drawSceneAtTime) بدون الاعتماد على أي ملف خارجي آخر.
...
### منهجية بناء scene.html (3 خطوات بالترتيب)
 ...
 3. **كتابة scene.html من الصفر**: دمج الطبقتين بالترتيب:
   * **طبقة الهوية**: منسوخة **حرفيًا** من ملف identities/<اسم>.md مع استبدال قيم بيانات المحتوى فقط بالبيانات الحقيقية للمهمة الحالية (الناتجة عن طلب curl حقيقي).
   * **الطبقة التقنية الثابتة**: منسوخة **حرفيًا** من القسم 2 أدناه دون أي تعديل.
### تحقق إلزامي بعد كتابة scene.html وقبل بدء الرندر
 1. **طباعة القيم الفعلية لبيانات المحتوى** (grep/cat على scene.html نفسه) والتأكد من مطابقتها للمطلوب في المهمة الحالية.
...
يتكون كل ملف scene.html من طبقتين متتاليتين:
 1. **🎨 طبقة الهوية (Identity Layer)**: منسوخة حرفياً من identities/<اسم>.md.
...
### ⚙️ الطبقة التقنية الثابتة (تنسخ حرفيًا في كل scene.html)
```html
<!DOCTYPE html>
...
<body>
    ...
    <script type="module">
        ...
        // ============ 🎨 طبقة الهوية بالكامل (منسوخة من identities/<اسم>.md) ============
