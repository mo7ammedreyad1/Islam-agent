...
تالتة صوت بس من غير أي نص خالص)، **ومنطق جلب أي أصول خاصة بيها** (صوت، صورة،
...
3. **اكتب `scene.html`** بدمج الطبقتين بالترتيب في القسم 2 — بفهم كامل لما بتكتبه
   (راجع "فهم عميق قبل الكتابة" تحت)، مش تجميع نصوص من غير وعي: كود الهوية
   بالكامل (**منسوخ حرفيًا** من ملف الـ `.md` بعد ما تفهمه، مش معاد صياغته أو
   "تحسينه" — إلا قيم بيانات المحتوى بس، اللي بتستبدلها بالقيم الحقيقية للمهمة
   الحالية نتيجة `curl` فعلية، راجع القسم 4)، ثم الطبقة التقنية الثابتة (**منسوخة
   حرفيًا** من القسم 2، بدون أي تعديل).
...
**لكن الفهم العميق مش بديل عن نسخ الكود المُختبَر حرفيًا، وده مقصود مش تراخي**:
كل سطر في الطبقة التقنية الثابتة، وكل سطر في كود الرسم/الـ CSS بتاع كل هوية
موجودة فعلًا في `identities/`، **مُختبَر فعليًا على تشغيلات حقيقية سابقة** — كتير
منه كان الحل المباشر لمشكلة موثّقة بالتفصيل في القسم 3 (قيمة `timeout` بالظبط،
قناة `chrome` بالتحديد، تسلسل الكودكس بالترتيب ده، أسماء الـ hooks بالحرف...).
مفيش قرار إبداعي مطلوب في الكود ده أصلًا — فإعادة صياغته من فهمك الخاص، حتى لو
كان فهمك صح 100%، بتجازف بإعادة إنتاج نفس الأخطاء دي من الصفر من غير أي فايدة
حقيقية تقابل الخطر ده. **الفهم بيوجّه إزاي تتعامل مع الكود وتربط بين أجزائه
وتشخّص أعطاله — مش سبب لإعادة كتابته في كل مرة.** الحالة الوحيدة اللي فيها
"كتابة من الصفر" فعلية ومطلوبة: هوية جديدة كليًا ملهاش كود مُختبَر موجود بعد
(القسم 1.5) — هناك فهمك العميق للعقد هو اللي بيوجّه كتابتك، لأن مفيش حاجة
"جاهزة" تنسخها أصلًا.
