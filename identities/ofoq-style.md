...
الكتلة دي **تنفيذ مرجعي** — افهمها كويس (الألوان، التخطيط، حركة الظهور،
الفينييت، Ken Burns)، وبعدين اكتب نسختك الخاصة بيك في "طبقة الهوية" جوه
`scene.html`: نفس السلوك، لكن بقيم بيانات المحتوى الحقيقية للمهمة الحالية
(نتيجة `curl` فعلية) وروابط صوت محلولة حرفيًا (راجع "قيم المحتوى تُكتب حرفية
دايمًا" في `AGENTS.md`)، مش نسخ للكتلة زي ما هي.
...
**تذكير مهم**: لما تكتب `prepareIdentity()` الفعلية في `scene.html` الحقيقي،
اكتب كل رابط `everyayah.com` بقيمته الحرفية الكاملة لكل آية على حدة (مش بـ
`${RECITER_ID}`/`${SURAH_NUMBER}` كمتغيرات محتوى) — الكود فوق نمط مرجعي يوضح
الفكرة بس، راجع "قيم المحتوى تُكتب حرفية دايمًا" في `AGENTS.md`.
