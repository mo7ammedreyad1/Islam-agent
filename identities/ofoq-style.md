...
>
> **هذا الملف هو النسخة الرابعة**. الفرق الوحيد عن النسخة الثالثة: خلفية
> الفيديو (`BACKGROUND_VIDEO_URL`) بقت **حقل بيانات إلزامي يملأه الـ Agent في
> كل مهمة** (زي `SURAH_VERSES` بالظبط) بدل ما كانت صورة اختيارية معطّلة
> بـ `null` بشكل دائم. التفاصيل الكاملة والسبب موثّقين في "ملاحظات معروفة"
> آخر الصفحة.
...
**تحديث النسخة الرابعة (هذا الملف) — سبب التعديل**: في النسخة الثالثة كان
