...
> **نشأة الهوية دي ونسختها**: اتبنت بمنطق "استيعاب ستايل فقط" (القسم 1.5 في
...
> النسخة الثانية والدقيقة** لهذه الهوية — استُخرجت بمراجعة كل سطر في الملف
> الأصلي وقيمة كل لون/رقم/دالة بالحرف (مش تقريب أو إعادة تخيّل). الفروق عن أي
> نسخة أقدم من هذا الملف موثّقة في "ملاحظات معروفة" آخر الصفحة.
...
نتيجة الفحص لا تغيّر القرار (استخراج ستايل فقط، زي أي ملف بيفشل بند واحد على
الأقل) — لكنها بتوضح إن فصل التصميم عن التنفيذ هنا كان مباشر نسبيًا لأن الملف
منظّم كويس أصلًا.
...
if (BACKGROUND_IMAGE_URL) {
    ...
    img.onerror = () => { logToConsole("تعذر تحميل صورة الخلفية، هيتم الاكتفاء بالتدرج اللوني.", "warn"); };
...
- **`getWorkingCodecConfig()`**: دالة كانت بتـ"تجرّب" كذا إعداد ترميز فيديو
  ...
  كودك. دي منطق تصدير/تنفيذ صرف — مستبعدة بالكامل حسب قاعدة "الكود التنفيذي ما
  بينتقلش أبدًا" في القسم 1.5. الطبقة التقنية الثابتة في `AGENTS.md` بتحقق نفس
  الهدف (مرونة لو كودك مش مدعوم) بآلية تانية: قائمة محاولات مرتبة مع
...
- **مصدر الصوت (`quranicaudio.com`, ملف واحد للسورة كاملة) وتوقيت `RAW_CUES`
  اليدوي**: مستبعدين لأنهم يخالفوا قاعدة `everyayah.com` آية بآية + التوقيت
  المحسوب من مدة صوت حقيقية (بندين 4 و5 في فحص التوافق فوق).
- **`ensureFontsLoaded()`**: دالة كانت بتستنى `document.fonts.load(...)` قبل
  ...
  بين طبقة التصميم والطبقة التقنية (مفيش hook جاهز ليها في الطبقة التقنية
  الثابتة حاليًا). عمليًا مش بتبقى مشكلة لأن تحميل الصوت من `everyayah.com` في
  الطبقة التقنية بياخد وقت كافي (بيلف على كل آية لوحدها) بحيث خطوط Google Fonts
  بتكون خلصت تحميل قبل أول فريم فعلي بيتصدّر.
...
- **وضع الصورة الاختيارية (`BACKGROUND_IMAGE_URL`) متعطّل افتراضيًا**: لو مهمة
  ...
  قبل ما تستخدمه — لو مفيش، الكانفاس هيبقى "tainted" والتصدير هيفشل).
